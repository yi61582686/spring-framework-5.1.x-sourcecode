...
public class DependencyDescriptor extends InjectionPoint implements Serializable {
	...
	public Object resolveCandidate(String beanName, Class<?> requiredType, BeanFactory beanFactory)
		...
		/**
		 * 这个方法是要去解析一个被依赖作为属性的bean。
		 *
		 * 这里调用getBean()方法，是在初始化一个bean（称为A）的过程中调用的。当实例化 A 后，还需要为A内部的属性赋值。
		 * 而 A 的属性是另一个bean（称为B），A 对 B 有依赖关系，所以需要解析出 B 类，才能完成对A的初始化。
		 *
		 * beanName：A 中的属性，即被 A 依赖的bean的名字，也就是 B 的名字。
		 */
