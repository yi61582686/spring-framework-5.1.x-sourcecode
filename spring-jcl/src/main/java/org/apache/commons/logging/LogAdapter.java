...
final class LogAdapter {
	...
	static {
		if (isPresent(LOG4J_SPI)) { // 判断是否存在log4j的jar包
			if (isPresent(LOG4J_SLF4J_PROVIDER) && isPresent(SLF4J_SPI)) {// 存在log4j转slf4j的provider转接器的jar包，并且存在slf4j的jar包
				// log4j-to-slf4j bridge -> we'll rather go with the SLF4J SPI;
				// however, we still prefer Log4j over the plain SLF4J API since
				// the latter does not have location awareness support.
				logApi = LogApi.SLF4J_LAL; // 使用slf4j
			}
			else { // 无法转换为 slf4j
				// Use Log4j 2.x directly, including location awareness support
				logApi = LogApi.LOG4J; // 使用slf4j
			}
		}
		else if (isPresent(SLF4J_SPI)) { // 只存在 slf4j 的jar包，且是org.slf4j.spi.LocationAwareLogger
			// Full SLF4J SPI including location awareness support
			logApi = LogApi.SLF4J_LAL; // 使用 slf4j
		}
		else if (isPresent(SLF4J_API)) { // 只存在 slf4j 的jar包，且是org.slf4j.Logger
			// Minimal SLF4J API without location awareness support
			logApi = LogApi.SLF4J;
		}
		else { // 不存在其他日志jar包，使用jul：java.util.log
